...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _init() async {
    ...
    _updatesSub = ChatService.listenForUpdates(widget.rideId).listen((_) {
  ...
  Future<void> _loadMessages({bool showLoader = true}) async {
    ...
    final result = await ChatService.fetchMessages(widget.rideId, _token);
    if (!mounted) return;
    setState(() {
      _loading = false;
  ...
  Future<void> _send() async {
    ...
    final result = await ChatService.sendMessage(widget.rideId, content, _token);
    if (!mounted) return;
  ...
  Widget _buildBubble(Message message) {
    final isMine = message.senderRole == _currentUserRole;
    return Align(
